...
apisix_runtime_repo="https://github.com/Crazy-xyr/apisix-build-tools.git"
...
build-apisix-deb:
ifeq ($(local_code_path), 0)
	git clone -b master $(apisix_repo) ./apisix
...
ifeq ($(app),apisix)
	git clone -b master $(apisix_runtime_repo) ./apisix-runtime
